...
const ManageAdmins = () => {
  ...
  const loadAdmins = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("http://localhost:5001/api/rdb/admins", {
        method: 'GET',
        headers: { 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingAdmin 
        ? `http://localhost:5001/api/rdb/admins/${editingAdmin.admin_id}`
        : "http://localhost:5001/api/rdb/admins";
  ...
  const handleDelete = async (adminId, adminName) => {
    if (window.confirm(`Are you sure you want to delete ${adminName}?`)) {
      try {
        const response = await fetch(`http://localhost:5001/api/rdb/admins/${adminId}`, {
          method: "DELETE",
          headers: { 
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
