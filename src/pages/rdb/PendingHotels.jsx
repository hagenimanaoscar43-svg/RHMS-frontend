...
const PendingHotels = () => {
  ...
  const handleApprove = async (hotelId) => {
    if (window.confirm('Approve this hotel?')) {
      try {
        const response = await fetch(`http://localhost:5001/api/rdb/hotels/${hotelId}/approve`, {
          method: "PUT",
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (response.ok) {
  ...
  const handleReject = async (hotelId) => {
    ...
    if (reason) {
      try {
        const response = await fetch(`http://localhost:5001/api/rdb/hotels/${hotelId}/reject`, {
          method: "PUT",
          headers: {
